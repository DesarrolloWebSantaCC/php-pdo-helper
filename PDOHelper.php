...
class PDOHelper {
	...
	/**
	 * Ejecuta una instrucción de MySQL.
	 *
	 * @param string $query
	 ...
	 *
	 * @param string $limit
	 *   Límite de número de filas recuperadas.
	 *
	 * @return array|bool
	 *   Matriz de filas solicitadas si funciona, FALSE si no.
	 */
	...
	public function insertEx( $tabla, $datos, $op = "", $values = array() ) {
		...
		$c   = "";
		foreach ( $datos as $clave => $dato ) {
			if ( strlen( $op ) > 0 &&  in_array( $clave, $values ) ) {
				$sql .= $c . $op . "(:" . $clave . ")";
			} else {
				$sql .= $c . ":" . $clave;
			}
	...
	public function delete( $tabla, $datos ) {
		...
		try {
			...
			$this->msg = "Error: no ha sido posible borrar el registro\n" . $e->getMessage();
